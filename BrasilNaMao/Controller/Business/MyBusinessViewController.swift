...
class MyBusinessViewController: UIViewController,UICollectionViewDelegate, UICollectionViewDataSource, UITextViewDelegate, UITextFieldDelegate  {
        ...
        if isFieldsWithValues() {
            ...
            Service.shared.getCoordinateFromGeoCoder(address: "\(number) \(street), \(city), \(province) \(postalCode)") { (coordinate, error) in
                ...
                if error == nil {
                    ...
                    let business = Business(id:"", description: description, name: name,rating: 0.0, address: address, contact: contact, creationDate: Service.shared.getTodaysDate(), category: category, user_id: (UIApplication.shared.delegate as! AppDelegate).userObj.id, hours:self.dailyHoursArray, photosURL:["","",""])
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isNewBusiness! {
            ...
            if indexPathItemForImage != indexPath.item {
                
                cellPhoto.imageCellBusiness.kf.setImage(
                    with: URL(fileURLWithPath: self.businessDetails.photosURL![indexPath.item]),
                ...
                imageArrayForStorage.remove(at: indexPath.item)
                imageArrayForStorage.insert(cellPhoto.imageCellBusiness.image!, at: indexPath.item)
            } else{
                cellPhoto.imageCellBusiness.image = imageArrayForStorage[indexPath.item]
            }
        }
    ...
    @IBAction func btnWeekHourPressed(_ sender: UIButton) {
        ...
        let height = 280
        customView = WeekHour(frame: CGRect.init(x: 0, y: Int(self.view.frame.height)-height, width: 375, height: height))
    ...
    func isFieldsWithValues() -> Bool{
        ...
        guard self.tfPhone.text != "" else { self.showAlert(title: General.warning, message: "O campo telefone deve ser preenchido"); return false}
